...
class WithPcGameFragment : Fragment() {
    ...
    private fun handleGameState(gameState: GameState?, view: View) {
        when (gameState) {
            YOU_WIN -> Toast.makeText(requireContext(), "you win", Toast.LENGTH_SHORT).show()

            YOU_LOSE -> Toast.makeText(requireContext(), "pc wins", Toast.LENGTH_SHORT).show()

            EVEN -> Toast.makeText(requireContext(), "It's Even", Toast.LENGTH_SHORT).show()
